...
with st.sidebar.form("input_form", clear_on_submit=True):
    ...
    inc = st.number_input("수입(원)", value=0, min_value=0, step=1000, format="%d")
    exp = st.number_input("지출(원)", value=0, min_value=0, step=1000, format="%d")
    ...
    if not df.empty:
        ...
        with gc1:
            if m_df['지출'].sum() > 0:
                ...
                fig.update_traces(hovertemplate='%{label}<br>%{value:,.0f}원')
...
with tab3:
    if not df.empty:
        ...
        ac1.metric("총 수입", f"{df['수입'].sum():,}원")
        ac2.metric("총 지출", f"{df['지출'].sum():,}원")
        ac3.metric("누적 잔액", f"{(df['수입'].sum()-df['지출'].sum()):,}원")
        st.divider()
    ...
    with st.expander("➕ 이달의 자산 기록하기", expanded=True):
        with st.form("asset_input_form", clear_on_submit=False):
            ...
            with c_j:
                ...
                j_p = st.number_input("연금저축", value=0, step=100000, format="%d", key='jp')
                j_2 = st.number_input("청약", value=0, step=100000, format="%d", key='j2')
                j_3 = st.number_input("보통예금", value=0, step=100000, format="%d", key='j3')
                j_4 = st.number_input("국내주식", value=0, step=100000, format="%d", key='j4')
                j_5 = st.number_input("해외주식", value=0, step=100000, format="%d", key='j5')
            with c_h:
                st.markdown("**👩 정희**")
                h_1 = st.number_input("예적금", value=0, step=100000, format="%d", key='h1')
                h_p = st.number_input("연금저축", value=0, step=100000, format="%d", key='hp')
                h_2 = st.number_input("청약", value=0, step=100000, format="%d", key='h2')
                h_3 = st.number_input("청년도약계좌", value=0, step=100000, format="%d", key='h3')
                h_4 = st.number_input("보통예금", value=0, step=100000, format="%d", key='h4')
                h_5 = st.number_input("국내주식", value=0, step=100000, format="%d", key='h5')
            with c_s:
                st.markdown("**👶 수인**")
                s_1 = st.number_input("수인 국내주식", value=0, step=100000, format="%d", key='s1')
                s_2 = st.number_input("수인 해외주식", value=0, step=100000, format="%d", key='s2')
            with c_c:
                st.markdown("**🏠 공통**")
                c_1 = st.number_input("보증금/기타", value=0, step=1000000, format="%d", key='c1')
            
            if st.form_submit_button("💾 자산 현황 저장"):
                d_str = a_date.strftime('%Y-%m-%d')
                new_rows = [
                    [d_str, '지호', '예적금', j_1], [d_str, '지호', '연금저축', j_p], [d_str, '지호', '청약', j_2], [d_str, '지호', '보통예금', j_3], [d_str, '지호', '국내주식', j_4], [d_str, '지호', '해외주식', j_5],
